...
def overlap_qregions(refs, queries):
    ...
    for index in coordinates_sortindex:
        ...
        if index % 2 == 0:
            current_region += 1
            if current_region == 1:
                merged.append([coordinate,coordinate])
                
        else:
            current_region -= 1
            if current_region == 0:
                merged[-1][1] = coordinate

    totalsize = sum([x[1]-x[0] for x in refs+ queries])
    mergedsize = sum([x[1]-x[0] for x in merged])
    
    return totalsize-mergedsize, merged
...
def connectblocks(allgroups, scores):
    ...
    for index, group in enumerate(allgroups):
        ...
        if group[0] in gene_past and start - gene_past[group[0]][2] <= 10000 and end- gene_past[group[0]][3] < 120000 :
            
            dupsize, merged_qregions = overlap_qregions(gene_past[group[0]][4], qregions)
            
            if dupsize < 0.5*min(size, sum([x[1]-x[0] for x in gene_past[group[0]][4]])):
                            
                gene_past[group[0]][2] = end
                gene_past[group[0]][1] += size - (start - gene_past[group[0]][2])
                gene_past[group[0]][4] = merged_qregions
                ...
                gene_past[group[0]] = [score, size, end, start, qregions]
...
def querycontig_overlap2(allaligns):
    ...
    all_coordinates = [x for y in allaligns for x in y[:2]]
    all_aligned = [y[-2:] for y in allaligns ]
    ...
    allgroups = [[allaligns[x[0]][3], scores[x[0]]]+x[1:]+[x[2]-x[1]]+[[all_aligned[x[0]]]] for x in allgroups if x[2] - x[1] > 100]
...
def main(args):
    ...
    for (contig), indexes in eachcontig.items():
        ...
        usecols = subdata[[2,3,4,5,9,10,7,8]].values.tolist()
