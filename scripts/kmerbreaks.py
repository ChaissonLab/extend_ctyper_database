    ...
    for contig, regions in contigs.items():
        ...
        for index2 in indeces:
            ...
            if (sumdist <= lastdist and sumdist <= rowdist) :
                ...
                if len(mapped_chunk):
                    if  lastdist < 0.05 * min(work_norm, new_norm) :
...
def fixcontig(contig, contigdata, contigs, index_tosegment, thematrix, size):
    ...
    for sortindex, newindex in enumerate(indeces):
        ...
        oldsize = abs(index_tosegment[sortindex][1] - index_tosegment[sortindex][0]) 
        current_break = []
        skipcontigs = set([contig])
        for newindex in indeces[(sortindex):(sortindex+1)]:
            
            current_break.append(newindex)
            
            findmatchs = getmatch(current_break, contigs, thematrix,skipcontigs, size)
            
            findmatchs_ = []
            for match in findmatchs:
                
                matched = match[0]
                if len(matched) < 2:
                    continue

                allsegments = [index_tosegment[x] for x in matched]
                allends = [x for y in allsegments for x in y[:2]]
                newsize = max(allends) - min(allends)
                
                if abs(math.log(oldsize) - math.log(newsize)) > math.log(1.05):
                    continue
                
                results.append([tuple(current_break),tuple(match[0]),match[1]])
                #skipcontigs.update(set([x[-1] for x in findmatch]))
            
                #print(index_tosegment[newindex], allsegments, newsize, oldsize,match[1])
...
def fixall(contigs, thematrix, names, name_tocontig):
    ...
    index_tosegment = dict()
    
    for contig, regions in contigs.items():
        
        for region in regions:
            index_tosegment[region[-1]] = region[:2] + [contig]+ [region[-1]]
    ...
    for contig, regions in contigs.items():
        ...
        indeces = [x[-1] for x in regions]
        sizes = [abs(x[1]-x[0]) for x in regions]
        
        result = fixcontig(contig, contigs[contig], include_contigs, index_tosegment, thematrix, size)
...
def uniform_truncate(thematrix, names, contigs, name_tocontig):
    ...
    contigs = {name:sorted(segments) if name[0][-1] == '-' else sorted(segments,reverse = 1) for name, segments in contigs.items()}
