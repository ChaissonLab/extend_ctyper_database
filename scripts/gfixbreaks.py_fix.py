...
def makegraph(fastafile, folder, threads):
        ...
        for file in files:
                pass
                os.system("python {}/graphmake.py_fix.py -i {}  -d {} -t {} -l 0 -c 0 -f 0".format(script_folder, file, folder, threads))
...
def findbreaks(inputfile, assemsizes, outputfile):
...
def readcontigsinfo(inputfile, breakfile, assemsizes, refprefix = "NC_0609"):
        ...
        with open(breakfile, mode = 'r') as f:
                ...
                for index, line in enumerate(f):
                        ...
                        if start <= 1 or end >= assemsizes[contig]-1:
                                blacklist.add(f"loci_{index}")
...
def main(args):
        ...
        ifmakegraph = 0
        if ifmakegraph == 1:
                findbreaks(args.input, scafsizes, args.output+"_loci.txt")
        ...
        regions = uniformbreaks(args.input, args.output+"_loci.txt", haplomergefile+"_allgraphalign.out", scafsizes, args.ref)
