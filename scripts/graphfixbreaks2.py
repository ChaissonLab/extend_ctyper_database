...
def process_locus(index, line, haplopath, folder, outputfile):
	...
	line[2] = max(0,line[2])
	header = ">loci_{}\t{}:{}-{}{}\t{}".format(index, contig, max(0,line[2]), line[3], line[1], line[4],line[5])
...
class graphDB:
	...
	def assemblysmall(self,allresults):
				...
				if size < 10000 and size > 1000:
					...
					if i == 0:
						...
						if bindrecords[bindindex2] > bindrecords[bindindex1]:
							bindrecords[bindindex2] += 1
						elif bindrecords[bindindex2] < bindrecords[bindindex1]:
							bindrecords[bindindex1] += 1
						elif abs(results[i+1][4] - results[i+1][3]) >= abs(results[i-1][4] - results[i-1][3]):
							bindrecords[bindindex2] += 1
						else:
							bindrecords[bindindex1] += 1
		...
		for loci,results in allresults.items():
			...
			ifnewbind = 1
			while ifnewbind == 1 and len(results) > 1:
				...
				ifnewbind = max(ifnewbind, any(x for x in connects if x == 1))
...
def getblocks(gbreaks, break_uniformed, contigspans, genetoscaff, graphinfo):
	...
	results = thegraph.assemblysmall(results)
