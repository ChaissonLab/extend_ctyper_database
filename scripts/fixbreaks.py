...
def truncatefix(outputfile):
	seqsizes = cl.defaultdict(int)
	name = ""
	blacklist = set()
	# First pass: compute sequence lengths
	with open(outputfile, mode='r') as f:
		for line in f:
			if line.startswith(">"):
				name = line.split()[0][1:]
			else:
				seqsizes[name] += len(line.strip())
				
	# Second pass: rewrite headers with updated end positions
	with open(outputfile, mode='r') as f, open(outputfile + "_fix", mode='w') as w:
		for line in f:
			if line.startswith(">"):
				name = line.split()[0][1:]
				...
				if match:
					...
					newend = start + seqsizes[name]
					if (end - newend) > 10 or start < 10:
						blacklist.add(name)
					# Replace end with newend in the header
					line = re.sub(r":(\d+)-\d+", fr":\1-{newend}", line)
			w.write(line)
			
	os.replace(outputfile + "_fix", outputfile)
	return blacklist
...
def makenewfasta(locifile, queryfile, outputfile, folder, threads):
	...
	blacklists = truncatefix(outputfile) 
	return blacklists
def findbreaks(inputfile, outputfile):
	...
	with open(outputfile, mode = 'w') as w:
		for (scarf, index), regions in new_contigs.items():
			...
			w.write("{}\t{}\t{}\t{}\t{}\n".format(locus,scarf[-1],max(0,start-cutoffdistance),(end+cutoffdistance),names))
...
def getspan_ongraph(pathes, cigars,rposis, qposis):
	...
	return newcigars, rposis, qposis
...
def readlineargraph(linearfile):
	...
	with open(linearfile, mode = 'r') as f:
		...
		for line in f:
			...
			graphinfo[name],gregions, qregions  = getspan_ongraph(pathes, cigars, gregion, qregion)
...
def getseqsize(thefile):
	seqsize = cl.defaultdict(int)
	with open(thefile, mode = 'r') as f:
		...
		for line in f:
			if line.startswith('>'):
				...
				seqsize[name] += len(line.strip())
	return seqsize

def readcontigsinfo(inputfile, breakfile):
	seqsize = getseqsize(inputfile)
	genetoscaff = dict()
	with open(inputfile, mode = 'r') as f:
		for line in f:
			if line.startswith(">"):
	...
	with open(breakfile, mode = 'r') as f:
		...
		for index, line in enumerate(f):
			...
			start, end = max(0, start), min(start + seqsize[locusname], end)
...
class graphDB:
	...
	def assemblysmall(self, allresults):
		...
		for loci,results in allresults.items():
			...
			for i,result in enumerate(results):
				...
				if size < 10000:
					iffix.add(loci)
					self.smallchunks.add(tuple(sorted(result[1])))
		...
		for loci,results in allresults.items() :
			...
			for i,result in enumerate(results):
				...
				if tuple(sorted(result[1])) in self.smallchunks:
...
def getblocks(gbreaks, break_uniformed, contigspans, genetoscaff, graphinfo, blacklists):
	...
	iffix = 1
	while iffix:
		results, iffix = thegraph.assemblysmall(results)
...
def breaks_ongraph(genetoscaff, genetolocus, graphinfo, contigspan, blacklists):
	...
	results = getblocks(gbreaks, break_uniformed, contigspan, genetoscaff, graphinfo, blacklists)
...
def annotate_regions(breaksoncontigs, locuslocations, genetoscaff):
	...
	for loci, breaks in breaksoncontigs.items():
		...
		for segment in breaks:
			...
			segmentindex = tuple(sorted(segment[1]))
...
def main(args):
	...
	if len(args.folder)==0: 
		folder = args.output +  datetime.datetime.now().strftime("%y%m%d_%H%M%S/").replace("%","_")
	...
	graphref = args.output+"_breaks.txt.fasta"
	
	#pairs = findbreaks(args.input, args.output+"_breaks.txt")
	blacklists = set()
	#blacklists = makenewfasta(args.output+"_breaks.txt", args.query, graphref, folder ,args.threads)
	...
	regions = uniformbreaks(args.input, args.output, blacklists)
	print([x for x in regions if x[1].startswith("NC_")])
