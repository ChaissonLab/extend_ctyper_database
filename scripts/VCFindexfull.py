...
def load_regions_and_refprefixes(regionfile: str, samplename: str, refkey: str = "HG38_h1"):
    """
    region TSV columns:
...
def vcfindex(inputfile, regionfile, output, samplename, slop=3000, refkey="HG38_h1"):
    regions_by_contig, refprefix_by_contig = load_regions_and_refprefixes(
        regionfile, samplename, refkey=refkey
    )
        ...
        for qry_contig, records in regions_by_contig.items():
            ...
            for ridx, rec in enumerate(records):
                ...
                w.write(
                    ...
                    f"{rec['strd']}\t{rec['score']}\t{rec['left']}\t{rec['right']}\t{result}\n"
                )
                
                def main(args):
    vcfindex(args.input, args.region, args.output, args.sample, slop=args.slop, refkey=args.refkey)
