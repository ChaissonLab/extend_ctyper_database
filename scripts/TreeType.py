...
class node:
	...
	def findclades(self, dm , rcutoff = 0.000, fcutoff = 3, mcutoff = 31*5, rcutoff2 = 0.005 * 3, mcutoff2 = 31*5 * 3):
		
		if len(self.children) == 0:
			return []
		
		clades = sum([child.findclades(dm, rcutoff, fcutoff, mcutoff, rcutoff2, mcutoff2) for child in self.children], [])
...
def normtotdm(matrix):
	...
	for i in range(len(matrix)):
		...
		for j in range(i+1):
			...
			dm [i,j] = 1 - matrix[i,j]/(math.sqrt(var1)*math.sqrt(var2))
			dm [j,i] =  dm [i,j]
			
	return dm

def loadnorm(filepath):
    ...
    with gzip.open(filepath,'rb') as f:
        ...
        for row in f:
            ...
            data[index,:] = [0]*(dim- len(row))+[float(x) for x in row]
...
def main(args):
	...
	normmatrix = loadnorm(normfile)	
	#dm = normtotdm(normmatrix)	
	headers = cl.defaultdict(str)
	...
	groups = [groups[x] for x in range(max(groups.keys())+1)]
	laststep1_groups = -1
	step1_index = -1
	lindex = 0
	with open(outputfile, mode = 'w') as f:
		...
		for i,group in enumerate(groups):
			...
			if len(group):
				...
				f.write(str(lindex)+"\t"+str(lindex )+"\t"+ ",".join([x for x in group if headers[x].split(":")[0] not in excludenames]) +"\n" )
				lindex += 1
...
def run():
	...
	parser.add_argument("-r", "--rcutoff", help="path to output file", dest="rcutoff", type=float, default= 0.005 )
	parser.add_argument("-f", "--fcutoff", help="path to output file", dest="fcutoff", type=float, default= 2.5 )
	parser.add_argument("-m", "--mcutoff", help="path to output file", dest="mcutoff", type=float, default= 31 * 7 )
	parser.add_argument("-r2", "--rcutoff2", help="path to output file", dest="rcutoff2", type=float, default= 0.005 * 5 )
	parser.add_argument("-m2", "--mcutoff2", help="path to output file", dest="mcutoff2", type=float, default= 31 * 10 * 2.6 )
